 ...
 *
 * NOTE: wrangler.jsonc is auto-generated from wrangler.template.jsonc
 * by the test:e2e npm script before vitest runs.
 */
export async function getTestWorkerUrl(): Promise<{
  ...
  console.log(
    `[WranglerRunner] TEST_WORKER_URL=${process.env.TEST_WORKER_URL ?? '<unset>'}`
  );

  // CI mode: use deployed worker URL
  if (process.env.TEST_WORKER_URL) {
    console.log(
      `[WranglerRunner] Using deployed test worker: ${process.env.TEST_WORKER_URL}`
    );
    return { url: process.env.TEST_WORKER_URL, runner: null };
  }

  // Local mode: spawn wrangler dev for this test file
  // Config is auto-generated by test:e2e script before vitest runs
  const testWorkerDir = 'tests/e2e/test-worker';
  console.log('[WranglerRunner] Spawning local wrangler dev...');
