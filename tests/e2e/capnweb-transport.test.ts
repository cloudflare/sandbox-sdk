 ...
 * - Session isolation
 *
 * When run with SANDBOX_TRANSPORT=capnweb in the test worker config,
 * all requests flow through the capnweb WebSocket RPC bridge instead
 * of the default HTTP transport.
 *
 * When run with default HTTP transport, these tests serve as additional
 * regression coverage for core operations.
 */
