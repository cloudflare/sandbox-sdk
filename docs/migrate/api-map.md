...
| `keepAlive`, `setKeepAlive()`                                                    | Pattern | Alarm that calls `setInactivityTimeout()` again               | [Coding agents example](../../examples/coding-agents).                |
...
| 0.12                                                           | Kind    | Replacement                      | Notes                                                                           |
| -------------------------------------------------------------- | ------- | -------------------------------- | ------------------------------------------------------------------------------- |
| `startProcess()`                                               | Native  | `container.exec()`               | Redirect output to a file. The handle is lost when the Durable Object restarts. |
| `Process.kill()`, `killProcess()`                              | Native  | `ExecProcess.kill()`             | Only while you hold the handle.                                                 |
| `Process.waitForExit()`                                        | Native  | `ExecProcess.exitCode`           |                                                                                 |
| `Process.waitForPort()`                                        | Pattern | Retry `getTcpPort(port).fetch()` | [Preview a web app](../preview-a-web-app.md).                                   |
| `Process.getStatus()`, `getProcess()`, `listProcesses()`       | None    | —                                |                                                                                 |
| `getProcessLogs()`, `streamProcessLogs()`, `Process.getLogs()` | None    | —                                | Write output to a file and read it with `files.readFile()`.                     |
| `killAllProcesses()`, `cleanupCompletedProcesses()`            | None    | —                                |                                                                                 |
| `Process.waitForLog()`                                         | Removed | —                                |                                                                                 |
| `processId`, `autoCleanup`, `onStart`, `onExit`, `onOutput`    | Removed | —                                |                                                                                 |
