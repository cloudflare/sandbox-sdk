...
| `startAndWaitForPorts()`, `waitForPort()`                                        | Pattern | Retry `getTcpPort(port).fetch()`                              | [Share a port](../share-a-port.md#2-start-the-server).                                                    |
...
| `Process.waitForPort()`                                  | Pattern | Retry `getTcpPort(port).fetch()`                            | [Share a port](../share-a-port.md#2-start-the-server).                                          |
...
| 0.12                                                                                                  | Kind    | Replacement                                                   | Notes                                                                                             |
| ----------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `exposePort()`, `unexposePort()`, `getExposedPorts()`, `isPortExposed()`                              | Pattern | Tokens in Durable Object storage                              | The share example keeps 0.12's hostname format. [Preview URLs and tunnels](ports-and-tunnels.md). |
| `validatePortToken()`                                                                                 | Pattern | A token check in the Durable Object's `fetch()`               | [Preview URLs and tunnels](ports-and-tunnels.md).                                                 |
| `proxyToSandbox()`, `SandboxEnv`                                                                      | Pattern | Route by hostname to `getByName()`                            | [Preview URLs and tunnels](ports-and-tunnels.md).                                                 |
| `wsConnect(request, port)`                                                                            | Native  | `container.getTcpPort(port).fetch(request)`                   | WebSockets pass through.                                                                          |
| `tunnels.get(port)`, `tunnels.list()`, `tunnels.destroy()`                                            | Pattern | `cloudflared tunnel --url` in the Container                   | Needs `enableInternet: true`. [Preview URLs and tunnels](ports-and-tunnels.md).                   |
| `tunnels.get(port, { name })`                                                                         | Pattern | Preview URLs, or `cloudflared tunnel run` with a tunnel token | Create one tunnel for each sandbox. [Preview URLs and tunnels](ports-and-tunnels.md).             |
| `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_TUNNEL_ACCOUNT_ID`, `CLOUDFLARE_ZONE_ID` | Removed | Your own secrets, if you create tunnels from the Worker       | [Preview URLs and tunnels](ports-and-tunnels.md).                                                 |
...
| 0.12                                                                                                                                             | Kind    | Replacement                                                   | Notes                                                                                                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------ | ------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `outbound`, `outboundByHost`, `outboundHandlers`, `outboundProxies`, `outboundProxy`                                                             | Pattern | One `WorkerEntrypoint` that picks a handler from stored rules | [Outbound rules](outbound-network.md).                                                                            |
| `setOutboundHandler()`, `setOutboundByHost()`, `setOutboundByHosts()`, `removeOutboundByHost()`                                                  | Pattern | Change the stored rules                                       | The next request follows them. [Outbound rules](outbound-network.md).                                             |
| `interceptHttps`                                                                                                                                 | Native  | `container.interceptOutboundHttps("*", fetcher)`              | Pass the certificate variables to `exec()`. [Outbound rules](outbound-network.md).                                |
| `allowedHosts`, `deniedHosts`, `setAllowedHosts()`, `setDeniedHosts()`, `allowHost()`, `denyHost()`, `removeAllowedHost()`, `removeDeniedHost()` | Pattern | `allow` and `deny` in the stored rules                        | Keep `enableInternet: false`. [Outbound rules](outbound-network.md).                                              |
| `ContainerProxy`                                                                                                                                 | Removed | Export your own entrypoint                                    | [Outbound rules](outbound-network.md).                                                                            |
...
| 0.12                                                                                                             | Kind    | Replacement                                                                        | Notes                                                                                          |
| ---------------------------------------------------------------------------------------------------------------- | ------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `mountBucket()` with an S3 endpoint                                                                              | Package | `S3Mounts.mount()` with `S3Gateway`                                                | Credentials stay in the Worker. [Bucket mounts](bucket-mounts.md).                             |
| `prefix`, `readOnly`, `s3fsOptions`                                                                              | Package | `keyPrefix`, `access`, `s3fsOptions`                                               | `keyPrefix` has no leading `/`. `s3fsOptions` is an object. [Bucket mounts](bucket-mounts.md). |
| `credentials`, `R2_*` and `AWS_*` variables                                                                      | Package | `S3Gateway` credentials                                                            | Not read from the environment. [Bucket mounts](bucket-mounts.md).                              |
| `credentialProxy`, `provider`                                                                                    | Removed | Credentials always stay in the Worker. Pass `nomixupload` in `s3fsOptions` for R2. | [Bucket mounts](bucket-mounts.md).                                                             |
| `mountBucket()` with an R2 binding                                                                               | Package | `S3Mounts.mount()` with `S3Gateway` and R2's S3 endpoint                           | Use an R2 API token. [Bucket mounts](bucket-mounts.md).                                        |
| `localBucket`                                                                                                    | Package | `S3Mounts` under `wrangler dev` with a local S3 server                             | Needs Wrangler 4.137.0 or later. [Develop locally](../s3-mounts.md#develop-locally).           |
| `unmountBucket()`                                                                                                | Package | `S3Mounts.unmount()`                                                               | [Bucket mounts](bucket-mounts.md).                                                             |
