...
Git is now an opt-in extension at `@cloudflare/sandbox/git` instead of built-in `sandbox.gitCheckout()` or `session.gitCheckout()` methods. Git operations are plain shell commands, so the extension drives them through the unified `exec()` process surface — there is no sidecar, and the container no longer ships a git service.
