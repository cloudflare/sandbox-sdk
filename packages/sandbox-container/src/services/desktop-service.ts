...
export class DesktopService {
  ...
  async stop(): Promise<ServiceResult<DesktopStopResult>> {
    try {
      // Terminate the worker thread BEFORE killing X11 processes.
      // The worker holds an FFI connection to X11 via robotgo. If we
      // kill Xvfb first, the dangling X11 connection can cause a
      // segfault in the FFI layer that takes down the entire Bun process.
      // With the worker stopped, no FFI calls are in flight when X11 dies.
