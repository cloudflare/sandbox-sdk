...
export class Container {
  ...
  async initialize(): Promise<void> {
    ...
    // Only use SessionManager in production (requires built control-process.js)
    // In test environment, ProcessService will fall back to direct Bun.spawn
    const isTest = process.env.NODE_ENV === 'test' || typeof Bun === 'undefined';
    const sessionManager = isTest ? undefined : new SessionManager(logger);
