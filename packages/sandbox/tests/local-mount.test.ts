...
describe('Sandbox local bucket mounts', () => {
  ...
  it('preserves a replacement mount created during stop cleanup', async () => {
    const sandbox = createSandbox();
    await sandbox.mountBucket('WORKSPACE', '/mnt/data', { localBucket: true });
    ...
    vi.spyOn(internal.currentRuntime, 'clear').mockImplementation(async () => {
    ...
    await sandbox.mountBucket('WORKSPACE', '/mnt/data', { localBucket: true });
    const replacement = internal.activeMounts.get('/mnt/data');
    expect(replacement?.containerGeneration).toBe(internal.containerGeneration);

    releaseRuntimeClear();
    await stop;

    expect(internal.activeMounts.get('/mnt/data')).toBe(replacement);
