...
import { CapnwebTransport, createTransport } from '../src/clients/transport';
...
describe('CapnwebTransport', () => {
  ...
  describe('request/response with mocked stub', () => {
    ...
    it('should make GET requests via the bridge stub', async () => {
      ...
      mockStub.httpFetch.mockResolvedValue({
        status: 200,
      ...
      expect(mockStub.httpFetch).toHaveBeenCalledWith(
        'GET',
        '/api/test',
        undefined
      );
    ...
    it('should throw for non-string body types', async () => {
      ...
      mockStub.httpFetch.mockResolvedValue({ status: 200 });
  ...
  describe('streaming with mocked stub', () => {
    it('should return ReadableStream from fetchStream', async () => {
      ...
      const mockStub = {
        httpFetch: vi.fn(),
        httpFetchStream: vi.fn().mockResolvedValue(mockStream),
        [Symbol.dispose]: vi.fn()
      };
      ...
      expect(mockStub.httpFetchStream).toHaveBeenCalledWith(
        'POST',
        '/api/execute/stream',
        JSON.stringify({ command: 'echo hello' })
      );
    ...
    it('should pass GET method for stream requests', async () => {
      ...
      const mockStub = {
        httpFetch: vi.fn(),
        httpFetchStream: vi.fn().mockResolvedValue(new ReadableStream()),
        [Symbol.dispose]: vi.fn()
      };
  ...
  describe('503 retry (inherited from BaseTransport)', () => {
    it('should retry on 503 and succeed on subsequent attempt', async () => {
      ...
      let callCount = 0;
      const mockStub = {
        httpFetch: vi.fn().mockImplementation(() => {
          callCount++;
          if (callCount === 1) {
      ...
      expect(response.status).toBe(200);
      expect(mockStub.httpFetch).toHaveBeenCalledTimes(2);
    });
