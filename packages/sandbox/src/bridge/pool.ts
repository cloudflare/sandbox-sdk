 ...
 */
export async function primePool(env: BridgeEnv, warmPoolBinding: string): Promise<void> {
  const warmTarget = Number.parseInt((env.WARM_POOL_TARGET as string) || '0', 10) || 0;
  const refreshInterval =
    Number.parseInt((env.WARM_POOL_REFRESH_INTERVAL as string) || '10000', 10) || 10_000;
