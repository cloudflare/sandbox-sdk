...
function createCommandSessionScript(tempDir: string): string {
  ...
  return [
    ...
    '__sandbox_sessions_emit_file() {',
    '  local exec_id="$1" stream="$2" file="$3" payload',
    '  if [[ -s "$file" ]]; then',
    ...
    '  __sandbox_sessions_emit_file "$exec_id" stdout "$stdout_file"',
    '  __sandbox_sessions_emit_file "$exec_id" stderr "$stderr_file"',
